...
function saveDeck(e) {
    ...
    fetch("/API/deckbuilder/saveDeck", {
...
function sentToJava(selectedClass, selectedDeck) {
    let obj = {"class": selectedClass, "deck": selectedDeck};
    fetch("/html-pages/deckDeckBuilder.html", {
        body: JSON.stringify(obj),
        method: 'POST'
    });
}
...
function setImgFunctions() {
    qsa("#cardsToChose img").forEach((item) => item.addEventListener('click', swap));
